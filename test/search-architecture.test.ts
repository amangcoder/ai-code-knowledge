...
describe('search_architecture handler', () => {
  ...
  it('returns line number prefixes in [architecture.md:N] format', async () => {
    ...
    const text = result.content[0].text;
    // Content lines within the architecture doc section should have the prefix
    const archLines = text.split('\n').filter((l) => l.startsWith('[architecture.md:'));
    expect(archLines.length).toBeGreaterThan(0);
  ...
  it('returns a no-results message when query has no matches', async () => {
    const result = await handler({ query: 'xyzzy_nonexistent_token' }, knowledgeRoot);
    expect(result.isError).toBeFalsy();
    expect(result.content[0].text).toContain('No results found');
  });

  it('returns a no-results message when architecture.md is missing', async () => {
