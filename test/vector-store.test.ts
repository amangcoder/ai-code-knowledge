...
describe('createVectorStore — with LanceDB mock', () => {
    ...
    it('isAvailable() returns false when VectorStore not initialized', async () => {
        // Mock LanceDB to throw to test the UnavailableVectorStore path
        const lancedbModule = await import('@lancedb/lancedb');
        const connectSpy = vi.spyOn(lancedbModule, 'connect' as never);
        // This test verifies the interface contract
        expect(typeof store => store.isAvailable()).toBe('function');
    });
