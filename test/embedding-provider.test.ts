 ...
 *
 * All HTTP calls to Ollama/OpenAI are mocked using vi.spyOn(global, 'fetch').
...
describe('OpenAIEmbeddingProvider', () => {
    ...
    function makeOpenAIResponse(texts: string[]): object {
        return {
            data: texts.map((_, i) => ({
    ...
    it('returns Float32Array[] from embed()', async () => {
        fetchSpy.mockResolvedValue(makeOpenAIResponse(['hello']) as unknown as Response);
    ...
    it('sends request to OpenAI embeddings endpoint', async () => {
        fetchSpy.mockResolvedValue(makeOpenAIResponse(['test']) as unknown as Response);
    ...
    it('sends multiple texts in a single API call', async () => {
        ...
        fetchSpy.mockResolvedValue(makeOpenAIResponse(texts) as unknown as Response);
    ...
    it('healthCheck() calls embed() with a short ping string', async () => {
        fetchSpy.mockResolvedValue(makeOpenAIResponse(['ping']) as unknown as Response);
...
describe('createEmbeddingProvider (factory)', () => {
    ...
    it('returns OllamaEmbeddingProvider by default (no EMBEDDING_MODEL set)', () => {
