...
describe('find_symbol (integration)', () => {
  it('finds createOrder and includes order-service in the file path', async () => {
    const result = await findSymbol({ name: 'createOrder' }, KNOWLEDGE_ROOT);
    expect(result.isError).toBeFalsy();
    const text = result.content[0].text;
    expect(text).toContain('createOrder');
    // The symbol's file is 'order-service.ts'; the path should contain the word
...
describe('health_check (integration)', () => {
  ...
  it('returns a human-readable status block with module list', async () => {
    ...
    // The fixture produces one module
    expect(text).toContain('sample-project');
  });
});
...
describe('search_architecture (integration)', () => {
  ...
  it('returns "No matching architecture content found" for an unmatched query (not-found path)', async () => {
  ...
  it('returns an error when architecture.md is absent', async () => {
