...
function parseArgs(): ParsedArgs {
    ...
    const rebuildFeatures = args.includes('--rebuild-features');
    ...
    // Exclude: collect all --exclude flags, each may be comma-separated
    const exclude: string[] = [];
    for (let i = 0; i < args.length; i++) {
        if (args[i] === '--exclude' && args[i + 1]) {
            exclude.push(...args[i + 1].split(',').map(s => s.trim()).filter(Boolean));
            i++;
        }
    }

    return { projectRoot, incremental, richness, skipVectors, skipFeatures, skipGraphify, rebuildFeatures, exclude };
}
...
async function main(): Promise<void> {
    const { projectRoot, incremental, richness: cliRichness, skipVectors, skipFeatures, skipGraphify, rebuildFeatures, exclude: cliExclude } = parseArgs();
    ...
    // ── Resolve exclude dirs: CLI > config ────────────────────────────────────
    const excludeDirs = [...(moduleConfig?.exclude ?? []), ...cliExclude];
    if (excludeDirs.length > 0) {
        log(`Excluding dirs: ${excludeDirs.join(', ')}`);
    }

    // ── Collect source files grouped by language ──────────────────────────────
    const filesByLanguage = collectSourceFiles(projectRoot, registry, excludeDirs);
    let totalFiles = 0;
    ...
    if (!skipVectors) {
        try {
            ...
            const embResult = await runEmbeddingPhase(knowledgeRoot, embeddingProvider, { incremental });
            log(`Embeddings: ${embResult.filesEmbedded} files, ${embResult.symbolsEmbedded} symbols in ${embResult.durationMs}ms`);
        } catch (err) {
            log(`Warning: Embedding phase failed — ${err instanceof Error ? err.message : err}`);
            log('Semantic search will be unavailable. Start embedding server: python scripts/embedding-server.py');
        }
    } else {
        log('Skipping embedding phase (--skip-vectors)');
    }
    ...
    try {
        ...
        log(`Graph: ${graphResult.nodeCount} nodes, ${graphResult.edgeCount} edges in ${graphResult.durationMs}ms`);
    } catch (err) {
        log(`Warning: Graph build phase failed — ${err instanceof Error ? err.message : err}`);
    }

    // ── Phase 8.5: Graphify enrichment ────────────────────────────────────────
    if (!skipGraphify) {
        try {
            const { runGraphifyPhase } = await import('./lib/phases/graphify-phase.js');
            const graphifyResult = await runGraphifyPhase(knowledgeRoot);
            if (graphifyResult.nodesAdded > 0 || graphifyResult.edgesAdded > 0) {
                log(`Graphify: +${graphifyResult.nodesAdded} nodes, +${graphifyResult.edgesAdded} edges in ${graphifyResult.durationMs}ms`);
            }
        } catch (err) {
            log(`Warning: Graphify enrichment phase failed — ${err instanceof Error ? err.message : err}`);
        }
    } else {
        log('Skipping graphify enrichment (--skip-graphify)');
    }
    ...
    if (!skipFeatures) {
        try {
            ...
            const { createEmbeddingProvider } = await import('./lib/embeddings/embedding-factory.js');
            const embeddingProvider = createEmbeddingProvider();
            const featureResult = await runFeatureDiscoveryPhase(knowledgeRoot, embeddingProvider, summarizer, { rebuildFeatures });
            log(`Features: ${featureResult.featuresDiscovered} discovered in ${featureResult.durationMs}ms`);
        } catch (err) {
