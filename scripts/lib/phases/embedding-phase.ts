...
export async function runEmbeddingPhase(
    ...
    // Cost projection constants (OpenAI pricing as of 2025)
    const PRICE_3_SMALL_PER_M = 0.020;  // text-embedding-3-small: $0.020/1M tokens
    const PRICE_ADA_PER_M = 0.100;      // text-embedding-ada-002: $0.100/1M tokens
    ...
    const costSmall = (estimatedTokens / 1_000_000) * PRICE_3_SMALL_PER_M;
    const costAda = (estimatedTokens / 1_000_000) * PRICE_ADA_PER_M;

    logInfo(
        PHASE,
        `Done: ${filesEmbedded} files, ${symbolsEmbedded} symbols, ` +
        `${skipped} skipped in ${durationMs}ms`
    );
    logInfo(
        PHASE,
        `Cost projection (if OpenAI API): ~${estimatedTokens.toLocaleString()} tokens` +
        ` → $${costSmall.toFixed(4)} (text-embedding-3-small) / $${costAda.toFixed(4)} (ada-002)`
    );
