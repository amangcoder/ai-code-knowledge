...
export interface EmbeddingPhaseResult {
    ...
    skipped: number;
    durationMs: number;
    /** Estimated token count based on character length (~4 chars/token). */
    estimatedTokens?: number;
    /** Projected cost in USD if OpenAI text-embedding-3-small had been used. */
...
/** The full knowledge graph data structure (nodes + edges). */
...
/** Arguments for the get_feature_context MCP tool. */
