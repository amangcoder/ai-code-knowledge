...
function parseArgs(argv) {
    ...
    const summarizer = summarizerArg?.split('=')[1] ?? 'static';
    const rootArg = args.find(a => a.startsWith('--root='));
    const projectRoot = rootArg?.split('=')[1] ??
        process.env['PROJECT_ROOT'] ??
        process.cwd();
    const knowledgeArg = args.find(a => a.startsWith('--knowledge='));
    const knowledgeRoot = knowledgeArg?.split('=')[1] ??
...
async function main() {
    ...
    process.stderr.write(`[build-knowledge] Project root: ${options.projectRoot}\n` +
...
main().catch((err) => {
    process.stderr.write(`[build-knowledge] Fatal error: ${err instanceof Error ? err.message : String(err)}\n`);
