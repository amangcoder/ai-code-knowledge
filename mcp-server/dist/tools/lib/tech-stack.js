    ...
    if (fs.existsSync(pkgPath)) {
        try {
            ...
            for (const dep of Object.keys(allDeps)) {
                if (FRAMEWORK_KEYWORDS[dep])
                    frameworks.add(FRAMEWORK_KEYWORDS[dep]);
                if (BUILD_TOOL_KEYWORDS[dep])
                    buildTools.add(BUILD_TOOL_KEYWORDS[dep]);
            }
...
export function classifyProjectType(projectRoot) {
    const pkgPath = path.join(projectRoot, 'package.json');
    if (!fs.existsSync(pkgPath)) {
        if (fs.existsSync(path.join(projectRoot, 'pyproject.toml')))
