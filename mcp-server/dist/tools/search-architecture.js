...
export async function handler(args, knowledgeRoot) {
    ...
    try {
        ...
        if (err.code === 'ENOENT') {
            return {
                content: [
                    {
                        type: 'text',
                        text: 'architecture.md not found. Run "npm run build-knowledge" to generate the knowledge artifacts, ' +
        ...
        return {
            content: [{ type: 'text', text: `Error reading architecture.md: ${err.message}` }],
            isError: true,
        };
    }
