...
function walkSourceFiles(projectRoot) {
    const results = [];
    function walk(dir) {
        if (results.length >= MAX_WALK_FILES)
            return;
        let entries;
        try {
 ...
 */
function findCoverageGaps(projectRoot, summaryCache) {
    ...
    for (const filePath of onDisk) {
        const relative = filePath
            ...
            .replace(/\\/g, '/')
            .replace(/^\//, '');
        const inCache = Object.keys(summaryCache).some(k => k === relative || k.endsWith('/' + relative) || relative.endsWith('/' + k));
        if (!inCache) {
            gaps.push(relative);
            if (gaps.length >= 20)
                break;
 ...
 */
function findStaleFiles(projectRoot, summaryCache, indexTimestamp) {
 ...
 */
function computeModuleCounts(summaryCache) {
    const counts = {};
    for (const filePath of Object.keys(summaryCache)) {
 ...
 */
function computeReadinessScore(gaps, stale, errors, richness) {
    let score = 100;
    score -= Math.min(gaps * 3, 30);
    score -= Math.min(stale * 2, 20);
    if (richness === 'minimal')
        score -= 20;
    else if (richness === 'standard')
        score -= 10;
...
export async function handler(args, knowledgeRoot = process.env['KNOWLEDGE_ROOT'] ?? '.knowledge') {
    ...
    if (gaps.length > 0) {
        ...
        if (gaps.length >= 20)
            gapLines.push('  (showing first 20)');
        sections.push({
    ...
    if (staleFiles.length > 0) {
        ...
        if (staleFiles.length > 20)
            staleLines.push(`  ... and ${staleFiles.length - 20} more`);
        sections.push({
