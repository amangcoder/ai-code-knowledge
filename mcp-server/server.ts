...
async function main(): Promise<void> {
    ...
    server.tool(
        ...
        'Get a complete project overview in a single call: file tree, tech stack, modules, symbol counts, ' +
        ...
        async (args: { depth?: number }) => {
            return withToolLogging('get_project_overview', args, () =>
                getProjectOverview.handler(args, KNOWLEDGE_ROOT));
        }
    );

    server.tool(
        'get_module_context',
        'Get everything about a module in one call: file summaries with architectural roles, exported symbols ' +
        ...
        async (args: { module: string }) => {
            return withToolLogging('get_module_context', args, () =>
                getModuleContext.handler(args, KNOWLEDGE_ROOT));
        }
    );

    server.tool(
        'get_implementation_context',
        'Get rich context for a single file before modifying it: purpose, all symbols with full signatures, ' +
        ...
        async (args: { file: string; includePatterns?: boolean }) => {
            return withToolLogging('get_implementation_context', args, () =>
                getImplementationContext.handler(args, KNOWLEDGE_ROOT));
        }
    );

    server.tool(
        'get_batch_summaries',
        'Get rich summaries for multiple files in a single call: description, richness level, architectural role, ' +
        ...
        async (args: { files: string[] }) => {
            return withToolLogging('get_batch_summaries', args, () =>
                getBatchSummaries.handler(args, KNOWLEDGE_ROOT));
        }
    );
    ...
    server.tool(
        'find_symbol',
        'Search for functions, classes, interfaces, and types by name. Uses case-insensitive substring matching. ' +
        ...
        async (args: { name: string; type?: string; module?: string }) => {
            return withToolLogging('find_symbol', args, () =>
                findSymbol.handler(args, KNOWLEDGE_ROOT));
        }
    );

    server.tool(
        'find_callers',
        'Find all symbols that call a given function/method using BFS traversal of the call graph. ' +
            ...
            'Example: find_callers(symbol="handler", direction="callees")',
        {
            symbol: z
                ...
                .max(500)
                .describe(
                    'Symbol name or "relativePath::symbolName" for unambiguous selection. ' +
            ...
            direction: z
                ...
                .optional()
                .describe(
                    '"callers" (default): who calls this symbol. "callees": what this symbol calls.'
                ),
        },
        async (args: { symbol: string; maxDepth?: number; direction?: 'callers' | 'callees' }) => {
            return withToolLogging('find_callers', args, () =>
                findCallers.handler(args, KNOWLEDGE_ROOT));
        }
    );

    server.tool(
        'get_dependencies',
        'Return direct and transitive module-level dependencies from the dependency graph. ' +
        ...
        async (args: { module: string; depth?: number }) => {
            return withToolLogging('get_dependencies', args, () =>
                getDependencies.handler(args, KNOWLEDGE_ROOT));
        }
    );

    server.tool(
        'get_file_summary',
        'Retrieve the AI-generated summary for a single source file: purpose, exports, dependencies, ' +
            ...
            'Example: get_file_summary(file="tools/lib/cache.ts")',
        {
            file: z
                .string()
                .describe(
                    'Relative file path — NOT absolute (e.g., "tools/lib/foo.ts" or just "cache.ts")'
                ),
        },
        async (args: { file: string }) => {
            return withToolLogging('get_file_summary', args, () =>
                getFileSummary.handler(args, KNOWLEDGE_ROOT));
        }
    );

    server.tool(
        'search_architecture',
        'Search across three sources: (1) architecture.md documentation, (2) file summaries ' +
        ...
        async (args: { query: string }) => {
            return withToolLogging('search_architecture', args, () =>
                searchArchitecture.handler(args, KNOWLEDGE_ROOT));
        }
    );

    server.tool(
        'health_check',
        'Check the health and readiness of the knowledge base. ' +
        ...
        async (args: { verbose?: boolean }) => {
            return withToolLogging('health_check', args, () =>
                healthCheck.handler(args, KNOWLEDGE_ROOT));
        }
    );
    ...
    server.tool(
        'get_artifact_schema',
        'Returns the expected JSON schema for a pipeline artifact type: required keys, key types, ' +
        ...
        async (args: { artifact_type: string }) => {
            return withToolLogging('get_artifact_schema', args, () =>
                getArtifactSchema.handler(args, KNOWLEDGE_ROOT));
        }
    );

    server.tool(
        'get_directory_tree',
        'Returns the current file/folder structure as a tree listing. ' +
        ...
        async (args: { path?: string; depth?: number }) => {
            return withToolLogging('get_directory_tree', args, () =>
                getDirectoryTree.handler(args, KNOWLEDGE_ROOT));
        }
    );

    server.tool(
        'get_artifact_store_path',
        'Returns the expected filesystem path where a pipeline artifact should be written. ' +
        ...
        async (args: { artifact_type: string }) => {
            return withToolLogging('get_artifact_store_path', args, () =>
                getArtifactStorePath.handler(args, KNOWLEDGE_ROOT));
        }
    );

    server.tool(
        'get_code_patterns',
        'Extracts recurring code patterns with occurrence counts and annotated examples: ' +
        ...
        async (args: { pattern_type?: string }) => {
            return withToolLogging('get_code_patterns', args, () =>
                getCodePatterns.handler(args, KNOWLEDGE_ROOT));
        }
    );

    server.tool(
        'find_template_file',
        'Given a description, finds the most similar existing files to use as templates for new code. ' +
        ...
        async (args: { description: string }) => {
            return withToolLogging('find_template_file', args, () =>
                findTemplateFile.handler(args, KNOWLEDGE_ROOT));
        }
    );

    server.tool(
        'get_static_data_schema',
        'Returns the structure of static data files: keys, exports, and relationships between data files. ' +
            'Use before reading or modifying static/config data files.',
        {},
        async () => {
            return withToolLogging('get_static_data_schema', {}, () =>
                getStaticDataSchema.handler({}, KNOWLEDGE_ROOT));
        }
    );

    server.tool(
        'validate_artifact_draft',
        'Pre-validates artifact JSON against the expected schema before final submission. ' +
        ...
        async (args: { artifact_type: string; json_content: string }) => {
            return withToolLogging('validate_artifact_draft', args, () =>
                validateArtifactDraft.handler(args, KNOWLEDGE_ROOT));
        }
    );
    ...
    server.tool(
        ...
        async (args: { query: string; scope?: 'files' | 'symbols' | 'features' | 'all'; topK?: number }) => {
            return withToolLogging('semantic_search', args, () =>
                semanticSearch.handler(args, KNOWLEDGE_ROOT));
        }
    );
    ...
    server.tool(
        ...
        async (args: {
            ...
            return withToolLogging('explore_graph', args, () =>
                exploreGraph.handler(args, KNOWLEDGE_ROOT));
        }
    );
    ...
    server.tool(
            ...
            'Example: get_cumulative_context(phase="backend_engineer")',
        {
            phase: z
                .string()
                .describe(
                    'Current pipeline phase name (e.g., "architecture", "engineering_plan", "backend_engineer", "qa")'
                ),
        },
        async (args: { phase: string }) => {
            return withToolLogging('get_cumulative_context', args, () =>
                getCumulativeContext.handler(args, KNOWLEDGE_ROOT));
        }
    );
