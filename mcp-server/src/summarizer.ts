...
import type { FileSummary, SymbolEntry } from './indexer.js';

export type SummarizerMode = 'static' | 'anthropic' | 'claude-code';

export interface SummarizerConfig {
    mode?: SummarizerMode;
    model?: string;       // e.g., 'claude-3-haiku-20240307'
    apiKey?: string;      // ANTHROPIC_API_KEY (falls back to env var)
 ...
 */
export function inferArchitecturalRole(relPath: string): string {
 ...
 */
export function generateStaticDescription(
    relPath: string,
    symbols: SymbolEntry[],
    imports: string[]
): string {
...
export async function generateLLMDescription(
    ...
    const prompt =
        `Analyze this source file and provide a 1-2 sentence description (100-200 chars) of its purpose.\n` +
        `File: ${relPath}\n` +
    ...
    try {
        ...
        process.stderr.write(`[summarizer] LLM call failed for ${relPath}: ${(err as Error).message}\n`);
    }
...
async function callAnthropicAPI(prompt: string, config: Required<SummarizerConfig>): Promise<string> {
...
async function callClaudeCode(prompt: string, config: Required<SummarizerConfig>): Promise<string> {
 ...
 */
export async function enrichSummaryWithLLM(
    summary: FileSummary,
    content: string,
    symbols: SymbolEntry[],
    config: SummarizerConfig = {}
): Promise<FileSummary> {
    const description = await generateLLMDescription(
        summary.file,
        content,
        symbols,
        config
 ...
 */
export function findTestFiles(relPath: string, allFiles: string[]): string[] {
