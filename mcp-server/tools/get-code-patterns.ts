...
const VALID_PATTERN_TYPES = ['component', 'css', 'data', 'routing', 'testing'] as const;
...
function classifyFile(filePath: string, summary: FileSummary): PatternType[] {
...
function formatFileEntry(filePath: string, summary: FileSummary, exampleSymbol?: SymbolEntry): string {
    const lines: string[] = [];
...
function countStructuralPatterns(symbols: SymbolEntry[], projectRoot: string): Section[] {
    ...
    const handlers = symbols.filter(s =>
        s.name === 'handler' || s.name.endsWith('Handler') || s.name.startsWith('handle')
    );
...
function detectDataAccessPattern(knowledgeRoot: string): string {
    const deps = loadDependencies(knowledgeRoot);
    if (!deps) return 'none detected';
    ...
    if (hasPrisma) return 'Prisma ORM';
    if (hasMongoose) return 'Mongoose/MongoDB';
    if (hasSql) return 'SQL (pg/mysql)';
    if (hasFs) return 'filesystem (fs module)';
    return 'none detected';
}

/** Detect dominant styling mechanism from dependencies. */
function detectStyling(knowledgeRoot: string, cache: Record<string, FileSummary>): string {
    const deps = loadDependencies(knowledgeRoot);
    if (!deps) {
    ...
    const allDeps = Object.values(deps.fileDeps ?? {}).flat();
    if (allDeps.some(d => d.toLowerCase().includes('tailwind'))) return 'Tailwind CSS';
    if (allDeps.some(d => d.toLowerCase().includes('styled-components'))) return 'styled-components';
    if (allDeps.some(d => d.toLowerCase().includes('emotion'))) return '@emotion';
    if (allDeps.some(d => d.toLowerCase().includes('sass'))) return 'Sass/SCSS';
    if (Object.keys(cache).some(f => f.endsWith('.css'))) return 'CSS files';
    return 'none detected';
}

export function handler(
    args: { pattern_type?: string },
    ...
    const requestedTypes: PatternType[] = args.pattern_type
        ? [args.pattern_type as PatternType]
        : [...VALID_PATTERN_TYPES];

    if (args.pattern_type && !VALID_PATTERN_TYPES.includes(args.pattern_type as PatternType)) {
        return {
            content: [{
                type: 'text',
    ...
    const grouped: Record<PatternType, Array<{ filePath: string; summary: FileSummary }>> = {
        component: [],
        css: [],
        data: [],
        routing: [],
