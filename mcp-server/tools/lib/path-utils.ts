...
export function findSummary(
    ...
    if (cache[normalizedInput]) return normalizedInput;
    ...
    for (const ext of extensions) {
        ...
        if (cache[withExt]) return withExt;
        const suffixMatch = keys.find(
            (key) => key === withExt || key.endsWith('/' + withExt),
        );
 ...
 * @param projectRoot Absolute project root path
 */
export function safePath(userInput: string, projectRoot: string): string | null {
...
export function computeClosestMatches(
    ...
    function score(candidate: string): number {
        ...
        if (candLower === queryLower) return 1.0;
        ...
        for (let i = 0; i <= candLower.length - 3; i++) {
            ...
            if (queryTrigrams.has(tri)) shared++;
        }
