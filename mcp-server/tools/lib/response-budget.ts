...
export interface Section {
    label: string;
    content: string;
    /** Lower = higher priority. 0=critical (always preserved), 1=core, 2=extended, 99=footer (first to drop) */
    priority: number;
}
 ...
 */
export function buildResponse(sections: Section[], toolBudget?: number): string {
    // Determine effective budget
    let budget = toolBudget ?? DEFAULT_BUDGET;
    if (budget > HARD_CAP) {
        process.stderr.write(
            `[response-budget] WARNING: requested budget ${budget} exceeds hard cap ${HARD_CAP}, clamping\n`
        );
        budget = HARD_CAP;
    }
...
function formatSectionBlock(section: Section): string {
    if (!section.label) {
