...
interface CacheEntry<T> {
    value: T;
    timestamp: string; // index.lastBuilt at time of caching
}
...
function parseMaxEntries(): number {
    ...
    if (raw === undefined || raw === '') return DEFAULT_MAX_ENTRIES;

    const parsed = parseInt(raw, 10);
    if (isNaN(parsed)) {
        process.stderr.write(
            `[cache] WARNING: KNOWLEDGE_CACHE_MAX_ENTRIES="${raw}" is not a number, using default ${DEFAULT_MAX_ENTRIES}\n`
        );
        return DEFAULT_MAX_ENTRIES;
    }
    if (parsed < MIN_ENTRIES || parsed > MAX_ENTRIES_LIMIT) {
        process.stderr.write(
            `[cache] WARNING: KNOWLEDGE_CACHE_MAX_ENTRIES=${parsed} out of range [${MIN_ENTRIES}-${MAX_ENTRIES_LIMIT}], using default ${DEFAULT_MAX_ENTRIES}\n`
        );
        return DEFAULT_MAX_ENTRIES;
    }
    return parsed;
...
function touch<T>(key: string, entry: CacheEntry<T>): void {
    store.delete(key);
    store.set(key, entry);
}
