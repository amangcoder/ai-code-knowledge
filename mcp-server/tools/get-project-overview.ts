...
import type { CallToolResult, SymbolEntry, DependencyGraph } from '../types.js';
import { loadIndex, loadSymbols, loadDependencies, loadSummaryCache, loadFeatureGroups } from './lib/data-loader.js';
...
function getMainEntry(projectRoot: string): string | null {
    const pkgPath = path.join(projectRoot, 'package.json');
    try {
        const pkg = JSON.parse(fs.readFileSync(pkgPath, 'utf8'));
...
function computeSymbolCounts(symbols: SymbolEntry[]): Record<string, number> {
...
function findEntryPoints(symbols: SymbolEntry[], mainEntry: string | null, projectRoot: string): string[] {
...
function detectLanguagesFromExtensions(projectRoot: string): string[] {
...
function buildModuleDependencyTable(
    ...
    for (const mod of modules) {
        const modSymbols = symbols.filter(s =>
            s.file.replace(/\\/g, '/').startsWith(mod + '/') ||
...
export function handler(
    ...
    // Key Features (from feature discovery, Phase 9)
    const features = loadFeatureGroups(knowledgeRoot);
    if (features && features.length > 0) {
        const featLines = features.map((f) =>
            `  - ${f.name}: ${f.description ?? ''}`.trimEnd()
        );
        sections.push({
            label: 'Key Features',
            content: featLines.join('\n'),
            priority: 7,
        });
    }
