...
import { loadSymbols, loadDependencies, loadSummaryCache, loadFileToSymbols, loadIndex } from './lib/data-loader.js';
...
function getSiblings(filePath: string, cache: Record<string, FileSummary>): string[] {
...
function classifyImports(deps: string[]): { node: string[]; external: string[]; relative: string[] } {
    const node: string[] = [];
...
function extractCodeSnippet(
    ...
    if (isSensitiveFile(absolutePath)) return null;

    try {
        const stat = fs.statSync(absolutePath);
        if (stat.size > MAX_SNIPPET_SOURCE_SIZE) return null;
...
export function handler(
    args: { file: string; includePatterns?: boolean },
    ...
    if (fileSymbols.length > 0) {
        ...
        const sigLines = filteredSymbols
            ...
            .map(s => {
                ...
                if (s.complexity != null) parts.push(`    Complexity: ${s.complexity}`);
                return parts.join('\n');
            });
    ...
    if (includePatterns) {
        ...
        if (deps?.fileDeps) {
            ...
            for (const [file, fileDeps] of Object.entries(deps.fileDeps)) {
                if (fileDeps.includes(key)) importedBy.push(file);
            }
        ...
        if (importClasses.node.length > 0) importStyles.push('node builtins');
        if (importClasses.external.length > 0) importStyles.push('external packages');
        if (importClasses.relative.length > 0) importStyles.push('relative imports');
