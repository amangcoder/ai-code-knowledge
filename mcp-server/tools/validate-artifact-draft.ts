...
export function handler(
  args: { artifact_type: string; json_content: string },
  ...
  try {
    parsed = JSON.parse(args.json_content) as Record<string, unknown>;
  } catch (err) {
    return {
      content: [
        {
          type: 'text',
          text: `JSON parse error: ${(err as SyntaxError).message}`,
        },
      ],
      isError: true,
    };
  ...
  if (args.artifact_type === 'review' && 'verdict' in parsed) {
    const validVerdicts = ['pass', 'fail', 'pass_with_warnings', 'approve', 'reject', 'request_changes'];
