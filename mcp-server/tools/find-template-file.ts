...
import type { CallToolResult, FileSummary, KnowledgeIndex } from '../types.js';
...
import type { VectorStore } from './lib/hybrid-retriever.js';
...
async function vectorSearch(
    ...
    try {
        ...
        process.stderr.write(
            '[find-template-file] EmbeddingProvider unavailable — falling back to token matching\n'
        );
    ...
    try {
        ...
        // Network / Ollama connectivity error: degrade gracefully
        process.stderr.write(
            '[find-template-file] Embedding request failed — falling back to token matching\n'
        );
        return tokenMatchSearch(description, knowledgeRoot, index, projectRoot);
    }
...
function tokenMatchSearch(
    ...
    const scored: Array<{ file: string; summary: FileSummary; score: number }> = [];

    for (const [filePath, summary] of Object.entries(cache)) {
        const combined =
            summary.purpose.toLowerCase() +
            ' ' +
